...
const _originalPathForIssueSuite = process.env.PATH
...
beforeEach(() => {
  ...
  // These tests only assert environment-agnostic text behavior. Keep them off
  // real git/gh probes so local CLI auth or network state cannot dominate time.
  process.env.PATH = ''
...
afterEach(() => {
  ...
  if (_originalPathForIssueSuite === undefined) {
    delete process.env.PATH
  } else {
    process.env.PATH = _originalPathForIssueSuite
  }
})
